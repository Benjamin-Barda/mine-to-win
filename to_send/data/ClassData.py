...
class ClassData(data.Dataset):
    ...
    def __init__(self, sources = [],  transform = None, transform_target = None, device = "cpu", train = True):
        ...
        self.images = np.asarray(
                [cv2.imread(os.path.join("imgs/frames/", i)) 
                for i in df.index]
                )
...
def create_and_save_dataset(srcs, filename):
    ...
    # To avoid problems with boxes: no flips or rotations!
...
def create_and_save_dataset_test(srcs, filename):
    ...
    # This is the test, no transforms!
    trans = transforms.Compose((
    transforms.ToTensor(),
    #transforms.RandomHorizontalFlip(),
    # transforms.ColorJitter(brightness=(0.6, 1.4), contrast=(0.95, 1.05), saturation=(0.99, 1.01)),
...
if __name__ == "__main__":
    ...
    # We removed empty images because of the domination of empty anchors!
    ...
    create_and_save_dataset(names, "data/datasets/minedata_compressed.dtst")

    names = [f"jsons/PARSEDOUTPUT-test{i}.json" for i in range(1, 3)]
    create_and_save_dataset_test(names, "data/datasets/minedata_compressed_test.dtst")
