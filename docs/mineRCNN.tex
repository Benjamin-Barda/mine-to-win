...
\IEEEpeerreviewmaketitle
    ...
    \section{Results}
    Given the inexperience, the difficulty of the task and the (inadequate) hardware at hand, we think to have reached positive results, the system shows signs of being able to recognize traits of the mobs we've trained it on, even though sometimes they are just cases of pareidolia: to give some numbers, the final model achieves a staggering $94\%$ accuracy on the validation set when recognizing whether an anchor contains an object or not. Is this a nice result? It depends, the real problem with this value is that, essentially, non-object anchors dominate the dataset (that's normal, unless you happen to be trying to classify every possible thing in an image), this 
