...
We decided to split the dataset into 80\% training and 20\% validation; We trained the network using SGD as suggested in \cite{arxiv:FasterRCNN} with a fixed learning rate of $10^-5$ and a momentum of $0.9$
    ...
    \begin{figure*}[htb]
        ...
        Going Further, we can see some pretty promising results in images (b), (f), (g), (c) and especially (d), in all of those the objects are almost always recognized by multiple rectangles at once, in our opinion this is due to the fact that, due to how the problem is mathematically formulated, a box does not have to recognize an object in its entirety, this makes the spreading of boxes over a single object a desirable goal for the network since it leads to a higher number of positives and therefore a higher score.
        ...
        \caption[Commenting Our Features]{We now look at five feature maps that we randomly chose from the \emph{Ziggurat}'s last output layer, as we can see, in the first row of filters, we get some clear activations 
